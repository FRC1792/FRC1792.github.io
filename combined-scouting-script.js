...
const ALLOWED_CODES = ["FRC3211"]; // Example: ["knights", "roundtable"]
...
function doGet(e) {
  ...
  return ContentService.createTextOutput(
    JSON.stringify({
      status: "ok",
      message: "FRC 3211 Combined Scouting Webhook (2026) is running",
...
function writeToSheetMatch(data) {
  ...
  // Parse timestamp
  const timestamp = data.timestampISO
    ? new Date(data.timestampISO)
    : new Date();
  ...
  const row = [
    ...
    data.autoFuelNeutralZone ? "Yes" : "No", // Auto - Fuel From Neutral Zone
    data.autoFuelOutpost ? "Yes" : "No", // Auto - Fuel From Outpost
    data.autoFuelDepot ? "Yes" : "No", // Auto - Fuel From Depot
    data.autoFuelOther ? "Yes" : "No", // Auto - Fuel From Other
    data.autoFuelNone ? "Yes" : "No", // Auto - No Intake
    ...
    // Teleop (Screen 2)
    data.teleopFuelRange || "", // Teleop Fuel (Active) Range
    ...
    data.teleopFuelOther ? "Yes" : "No", // Teleop - Fuel From Other
    data.teleopFuelNone ? "Yes" : "No", // Teleop - No Intake

    // Endgame (Screen 3)
    data.Tower || "NONE", // Endgame Tower Level
    data.climbPos || "", // Climb Position
    data.shotInHub || "", // Shot In Hub

    // Misc (Screen 4)
    data.defenseRating || "", // Defense Rating
    data.againstDefenseRating || "", // Against Defense Rating
    data.teleopBumpOver ? "Yes" : "No", // Crossed Bump
    data.teleopTrenchUnder ? "Yes" : "No", // Crossed Trench
    data.teleopBumpTrenchNone ? "Yes" : "No", // No Trench or Bump
    data.Feeder ? "Yes" : "No", // Feeder
...
function writeToSheetPit(data) {
  ...
  if (data.robotPhoto && data.robotPhoto.length > 0) {
    try {
      ...
      sheet
        .getRange(currentRow, photoColumn)
        .setValue("Photo upload failed: " + imageError.message);
    }
  } else {
  ...
  Logger.log(
    "✓ Pit row appended: Team " + data.teamNumber + " (" + data.teamName + ")",
  );
}
...
function getOrCreateImageFolder() {
  const folderName = "FRC 3211 Robot Photos";
...
function createHeadersMatch(sheet) {
  const headers = [
    ...
    "Start Position",
    "Auto Fuel Range",
    "Auto - Intake From Neutral Zone",
    "Auto - Intake From Outpost",
    "Auto - Intake From Depot",
    "Auto - Intake From Floor",
    "Auto - No Intake",
    "Auto Over Bump",
    "Auto Under Trench",
    "Auto Bump/Trench None",
    "Auto Tower",
    // Teleop (Screen 2)
    "Teleop Fuel Range",
    "Teleop - Intake From Neutral Zone",
    "Teleop - Intake From Outpost",
    "Teleop - Intake From Depot",
    "Teleop - Intake From Floor",
    "Teleop - No Intake",
    // Endgame (Screen 3)
    "Endgame Tower",
    "Climb Position",
    ...
    // Misc (Screen 4)
    "Defense Rating",
    "Against Defense Rating",
    "Teleop Over Bump",
    "Teleop Under Trench",
    "Teleop Bump/Trench None",
    "Feeder",
    "Comments",
  ];
...
function testMatchScouting() {
  ...
  const testData = {
    ...
    startPos: "1",
    autoFuelRange: "40-60",
    autoFuelNeutralZone: true,
    autoFuelOutpost: false,
    autoFuelDepot: true,
    autoFuelOther: false,
    autoFuelNone: false,
    autoBumpOver: true,
    ...
    // Misc
    defenseRating: "3",
    againstDefenseRating: "0",
    teleopBumpOver: true,
    teleopTrenchUnder: false,
    teleopBumpTrenchNone: false,
    Feeder: "No",
  ...
  try {
    ...
    Browser.msgBox(
      "Success!",
      "Check 'Match Scouting Data' tab for test data (Match 999, Team 3211)",
      Browser.Buttons.OK,
    );
...
function testPitScouting() {
  ...
  const testData = {
    ...
    ballCapacity: 12,
    specialFeatures:
      "Advanced autonomous routines, vision tracking system, dual intake mechanism",
  ...
  try {
    ...
    Browser.msgBox(
      "Success!",
      "Check 'Pit Scouting Data' tab for test data (Team 1792).\n\nYou should see:\n- A red square test image\n- '✓ Photo' text in the cell\n\nReal robot photos will be full-size.",
      Browser.Buttons.OK,
    );
...
function initializeSheets() {
  ...
  if (messages.length > 0) {
    ...
    Browser.msgBox(
      "Already Initialized",
      "Both sheets already exist with data.",
      Browser.Buttons.OK,
    );
...
function clearMatchData() {
  ...
  if (lastRow > 1) {
    ...
    Browser.msgBox(
      "Success",
      "✓ Cleared " + (lastRow - 1) + " rows from Match Scouting Data",
      Browser.Buttons.OK,
    );
  ...
  Browser.msgBox(
    "No Data",
    "No data to clear in Match Scouting Data",
    Browser.Buttons.OK,
  );
...
function clearPitData() {
  ...
  if (lastRow > 1) {
    ...
    Browser.msgBox(
      "Success",
      "✓ Cleared " + (lastRow - 1) + " rows from Pit Scouting Data",
      Browser.Buttons.OK,
    );
    return "✓ Cleared " + (lastRow - 1) + " rows";
  }
  ...
  Browser.msgBox(
    "No Data",
    "No data to clear in Pit Scouting Data",
    Browser.Buttons.OK,
  );
  return "No data to clear";
}
